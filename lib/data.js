...
// True when this client has approved a design on some earlier project. Used to
// collapse the walkthrough for a returning brand instead of making them scroll
// past instructions they have already followed.
//
// Scoped to other projects on purpose. Approving a design inside the current
// project must not collapse the card mid-session, since the page refreshes
// itself every minute and the block would vanish while they were using it.
export async function hasApprovedBefore(clientEmail, excludeProjectId) {
  if (!clientEmail) return false;
  try {
    const rows = await sql`
      SELECT 1
      FROM proof_approvals a
      JOIN proof_skus s ON s.id = a.sku_id
      JOIN proof_projects p ON p.id = s.project_id
      WHERE lower(p.client_email) = lower(${clientEmail})
        AND p.id <> ${excludeProjectId}
      LIMIT 1`;
    return rows.length > 0;
  } catch (e) {
    // Never block the portal over a cosmetic default.
    console.error('hasApprovedBefore failed:', e?.message || e);
    return false;
  }
}
